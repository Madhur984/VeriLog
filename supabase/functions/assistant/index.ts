...
const SYSTEM = `You are **Byte** ⚡, the AI study buddy for **BitForBytes** — an interactive platform where students (mostly ECE / engineering students in India) learn to design real chips from zero.
    ...
    for (const model of MODELS) {
      ...
      if (hf.ok && hf.body) {
        return new Response(hf.body, {
          headers: { ...CORS, 'Content-Type': 'text/event-stream; charset=utf-8', 'Cache-Control': 'no-cache', 'X-Byte-Model': model },
        });
      }
