...
export const KMapGroupingLab: React.FC<Props> = ({ onComplete, onInvalidGroup, saveSession }) => {
    ...
    return (
        <div className="relative flex flex-col gap-10 px-10 font-mono w-full max-w-5xl mx-auto">
            {/* Header */}
            ...
            {/* XP Tracker */}
            <motion.div animate={{ opacity: xpTotal > 0 ? 1 : 0 }} className="absolute -top-4 right-4 flex items-center gap-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 rounded-2xl px-5 py-3 text-xs font-black text-emerald-600 dark:text-emerald-400 shadow-xl shadow-emerald-500/10">
            ...
            <AnimatePresence>
                {feedback === 'success' && (
                    ...
                    >
                        ...
                        <div>
                            <div className="text-lg font-black text-slate-900 dark:text-white uppercase italic">Valid Group!</div>
                            <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">+{lastXP} structural XP synchronized</div>
                        </div>
                    </motion.div>
                )}
                ...
                {feedback === 'error' && (
                    ...
                    >
                        ...
                        <div>
                            <div className="text-lg font-black text-slate-900 dark:text-white uppercase italic">Invalid Group</div>
                            <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Groups must be powers of 2 (binary logic)</div>
                        </div>
                    </motion.div>
                )}
                ...
                {feedback === 'complete' && (
                    ...
                    >
                        ...
                        <div className="text-2xl font-black text-slate-900 dark:text-white uppercase italic tracking-widest">MINIMIZATION_COMPLETE</div>
                        <div className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest italic">Advancing to logical realization...</div>
                    </motion.div>
                )}
            </AnimatePresence>
        </div>
