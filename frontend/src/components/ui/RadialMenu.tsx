...
const RadialSegment: React.FC<SegmentProps> = ({ label, icon, startAngle, endAngle, radius, innerRadius, isActive, onClick, index, isLight }) => {
  ...
  return (
    ...
    >
      <motion.path d={basePath} className={cn("transition-colors duration-300 pointer-events-none", isActive ? (isLight ? "fill-[#7A3FD0]/[0.15]" : "fill-blue-950/90") : isHovered ? (isLight ? "fill-[#7A3FD0]/[0.10]" : "fill-indigo-950/90") : (isLight ? "fill-[#7A3FD0]/[0.12]" : "fill-[#020617]/[0.90]"))} />

      <motion.path d={topPath}
        className={cn("transition-colors duration-300",
          isActive
            ? (isLight ? "fill-[#7A3FD0]/[0.15] stroke-[#7A3FD0]/[0.70] stroke-2" : "fill-blue-900/70 stroke-blue-400 stroke-2")
            : isHovered
              ? (isLight ? "fill-[#7A3FD0]/[0.18] stroke-[#7A3FD0]/[0.55] stroke-[1.5]" : "fill-indigo-900/60 stroke-indigo-400 stroke-[1.5]")
              : (isLight ? "fill-[#7A3FD0]/[0.12] stroke-[#7A3FD0]/[0.45] stroke-[1.5]" : "fill-slate-800/80 stroke-slate-600/60 stroke-1")
        )}
        style={{
          filter: isActive
            ? (isLight ? 'drop-shadow(0 0 10px rgba(122,63,208,0.28))' : 'drop-shadow(0 0 15px rgba(59, 130, 246, 0.5))')
            : isHovered
              ? (isLight ? 'drop-shadow(0 0 8px rgba(122,63,208,0.22))' : 'drop-shadow(0 8px 12px rgba(0,0,0,0.6))')
              : (isLight ? 'drop-shadow(0 0 4px rgba(122,63,208,0.14))' : 'drop-shadow(0 4px 6px rgba(0,0,0,0.5))')
        }}
      />

      <motion.path d={describeArc(160, 160, radius - 1.5, innerRadius + 1.5, startAngle, endAngle)} className={cn("fill-none pointer-events-none", isLight ? "stroke-[#7A3FD0]/[0.30] stroke-[1.5]" : "stroke-white/10 stroke-1")} />

      {isActive && (
        <motion.path d={describeArc(160, 160, radius + 1, radius, startAngle, endAngle)} className={cn("pointer-events-none", isLight ? "fill-[#7A3FD0]/[0.40]" : "fill-blue-400")}
        ...
        <foreignObject x={iconPos.x - 45} y={iconPos.y - 35} width={90} height={75}>
          <div className="w-full h-full flex flex-col items-center justify-center gap-1.5 pt-1">
            <div className={cn("w-10 h-10 rounded-xl border flex items-center justify-center transition-colors duration-300 relative overflow-hidden",
              isActive
                ? (isLight ? "bg-[#7A3FD0]/[0.07] border-[#7A3FD0] text-[#7A3FD0] shadow-[0_0_15px_rgba(122,63,208,0.3)]" : "bg-blue-500/20 border-blue-400/50 shadow-[0_0_15px_rgba(59,130,246,0.5)] text-blue-300")
                : isHovered
                  ? (isLight ? "bg-[#7A3FD0]/[0.12] border-[#7A3FD0]/[0.55] text-black shadow-[0_0_12px_rgba(122,63,208,0.2)]" : "bg-indigo-500/20 border-indigo-400/40 text-indigo-300 shadow-xl")
                  : (isLight ? "bg-[#7A3FD0]/[0.07] border-[#7A3FD0]/[0.35] text-black" : "bg-slate-900/60 border-slate-600/50 text-slate-400 shadow-lg")
            )}>
              <div className="absolute top-0 inset-x-0 h-1/2 bg-gradient-to-b from-white/10 to-transparent pointer-events-none" />
              {icon}
            </div>
            <div className={cn("transition-colors font-black tracking-widest uppercase font-mono",
              isLight
                // no pill / oval — just black text on the lavender hologram
                ? "text-[11px] text-black"
                : cn("px-2 py-0.5 rounded-full border text-[9px] shadow-xl border-white/5",
                    isActive ? "bg-blue-900/90 text-blue-300 shadow-[0_0_10px_rgba(59,130,246,0.3)]"
                      : isHovered ? "bg-indigo-950/90 text-indigo-200"
                        : "bg-[#090e1a]/[0.90] text-slate-400")
            )}>
              {label}
            </div>
          </div>
...
const CenterButton: React.FC<{ isLight: boolean }> = ({ isLight }) => {
  ...
  return (
    ...
    >
      <circle cx="160" cy="180" r="85" fill={isLight ? "rgba(122,63,208,0.14)" : "rgba(0,0,0,0.7)"} filter="blur(16px)" className="pointer-events-none" />
      <circle cx="160" cy="168" r="86" className={isLight ? "fill-[#7A3FD0]/[0.12] pointer-events-none" : "fill-[#020617] pointer-events-none"} />

      <circle cx="160" cy="160" r="86"
        className={cn("transition-colors duration-300",
          isLight
            ? cn("stroke-[#7A3FD0]/[0.45]", isHovered ? "fill-[#7A3FD0]/[0.10]" : "fill-[#7A3FD0]/[0.15]")
            : cn("stroke-slate-500/50", isHovered ? "fill-blue-950/90" : "fill-slate-900/95")
        )}
        strokeWidth={isLight ? 2 : 2}
        style={{
          filter: isLight
            ? (isHovered ? 'drop-shadow(0 0 24px rgba(122,63,208,0.35))' : 'drop-shadow(0 0 16px rgba(122,63,208,0.28))')
      ...
      <circle cx="160" cy="160" r="72" className={isLight ? "fill-transparent stroke-[#7A3FD0]/[0.25] pointer-events-none" : "fill-transparent stroke-slate-700/40 pointer-events-none"} strokeWidth={14} />
      <motion.circle cx="160" cy="160" r="72" className={isLight ? "fill-transparent stroke-[#7A3FD0]/[0.45] pointer-events-none" : "fill-transparent stroke-blue-400 pointer-events-none"} strokeWidth={isLight ? 2.5 : 2} strokeDasharray="2 12" animate={{ rotate: 360 }} transition={{ duration: 40, repeat: Infinity, ease: "linear" }} style={{ originX: '160px', originY: '160px', filter: isLight ? 'drop-shadow(0 0 4px rgba(122,63,208,0.35))' : undefined }} />

      <g className={isLight ? "stroke-[#7A3FD0]/[0.55] pointer-events-none" : "stroke-slate-500/20 pointer-events-none"}>
      ...
      <motion.circle cx="160" cy="160" r="78" className={isLight ? "fill-transparent stroke-[#7A3FD0]/[0.45] pointer-events-none" : "fill-transparent stroke-blue-400 pointer-events-none"} strokeWidth={4.5} strokeDasharray="100 400" strokeLinecap="round" initial={{ strokeDashoffset: -100 }}
        animate={{ strokeDashoffset: -20, rotate: isHovered ? 15 : 0, filter: isHovered ? (isLight ? "drop-shadow(0 0 14px #7A3FD0)" : "drop-shadow(0 0 16px #60a5fa)") : (isLight ? "drop-shadow(0 0 8px #7A3FD0)" : "drop-shadow(0 0 8px #1d4ed8)") }}
      ...
      <foreignObject x={85} y={105} width={150} height={110} style={{ pointerEvents: 'none' }}>
        <div className="w-full h-full flex flex-col items-center justify-center p-2 text-center select-none">
          <div className={`font-mono uppercase tracking-[0.25em] mb-1 font-bold flex items-center justify-center gap-1.5 opacity-90 ${isLight ? 'text-[11px] text-black' : 'text-[9px] text-blue-400'}`}>
             <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${isLight ? 'bg-[#7A3FD0] shadow-[0_0_10px_rgba(122,63,208,0.6)]' : 'bg-blue-400 shadow-[0_0_8px_#60a5fa]'}`} /> OS Core V1
          </div>
          <div className={`font-black leading-tight tracking-wide uppercase ${isLight ? 'text-[17px] text-black' : 'text-[15px] text-white drop-shadow-[0_4px_8px_rgba(0,0,0,0.9)]'}`}>Hierarchical<br/>Grind</div>
          <div className={`mt-3 w-20 h-1.5 border rounded-full overflow-hidden shadow-inner flex ${isLight ? 'bg-[#7A3FD0]/[0.15] border-[#7A3FD0]/[0.40]' : 'bg-[#020617] border-slate-700/50'}`}>
             <motion.div className={`h-full relative ${isLight ? 'bg-[#7A3FD0]' : 'bg-blue-400'}`} initial={{ width: '0%' }} animate={{ width: '65%' }} transition={{ delay: 0.5, duration: 1.5, ease: "easeOut" }}>
                <div className={`absolute inset-0 ${isLight ? 'bg-[#7A3FD0] shadow-[0_0_12px_rgba(122,63,208,0.5)]' : 'bg-white/40 shadow-[0_0_12px_#3b82f6]'}`} />
             </motion.div>
          </div>
        </div>
      </foreignObject>
    ...
    >
       ...
       >
         ...
         >
             ...
             <div className="absolute inset-0 rounded-full pointer-events-none z-50" style={{ transform: 'translateZ(1px)', background: isLight ? 'linear-gradient(to bottom, rgba(255,255,255,0.18), transparent 60%)' : 'linear-gradient(to bottom, rgba(0,0,0,0.2), rgba(255,255,255,0.05))' }} />

             {/* Edge lighting - subtle bottom highlight simulating physical light catch */}
             <div className="absolute inset-0 rounded-full pointer-events-none z-50" style={{ transform: 'translateZ(1px)', borderBottom: isLight ? '1.5px solid rgba(122,63,208,0.3)' : '1px solid rgba(255,255,255,0.15)' }} />
             ...
             <svg width="320" height="320" viewBox="0 0 320 320" className="absolute inset-0 overflow-visible z-10" style={{ transform: 'translateZ(-10px)' }}>
                 <circle cx="160" cy="168" r="156" fill={isLight ? "rgba(233,230,251,0.9)" : "rgba(2, 6, 23, 0.9)"} />
                 <circle cx="160" cy="160" r="156" fill={isLight ? "rgba(233,230,251,0.92)" : "rgba(15, 23, 42, 0.85)"} stroke={isLight ? "rgba(122,63,208,0.3)" : "rgba(255,255,255,0.05)"} strokeWidth={isLight ? 1.5 : 1} />
              </svg>
