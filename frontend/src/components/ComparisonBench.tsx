...
const COMPARISON_ROWS = [
  ...
  },
  {
    ...
    winner: null // lower difficulty isn't necessarily better in AXE-OR context
  },
  {
    label: 'YoY GROWTH',
...
export const ComparisonBench: React.FC<ComparisonBenchProps> = ({ comparingIds, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[2100] flex items-center justify-center p-4 sm:p-8 bg-[#020408]/95 backdrop-blur-2xl">
      <motion.div
        initial={{ scale: 0.95, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        exit={{ scale: 0.95, opacity: 0 }}
        className="w-full max-w-5xl h-[85vh] bg-[#0D0F12] border border-white/10 rounded-2xl overflow-hidden flex flex-col shadow-2xl"
      >
        {/* Header */}
        <div className="p-6 sm:p-10 border-b border-white/[0.05] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h2 className="text-2xl font-bold text-white uppercase tracking-tight">Domain Comparison Bench</h2>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
           {/* Domain Headings */}
           <div className="grid grid-cols-10 border-b border-white/[0.05] bg-white/[0.01]">
              <div className="col-span-2 p-6" />
              <div className="col-span-4 p-8 border-x border-white/[0.05] text-center space-y-2">
                 <h3 className="text-white font-extrabold text-xl uppercase tracking-tight">{domainA.name}</h3>
              ...
              <div className="col-span-4 p-8 text-center space-y-2">
                 <h3 className="text-white font-extrabold text-xl uppercase tracking-tight">{domainB.name}</h3>
           ...
           <div className="divide-y divide-white/[0.03]">
              {COMPARISON_ROWS.map((row, idx) => {
                ...
                return (
                  <div key={idx} className="grid grid-cols-10 group hover:bg-white/[0.01] transition-colors">
                    ...
                    <div className="col-span-4 p-6 border-x border-white/[0.05] flex items-center justify-center relative">
                       {winner === 'a' && <div className="absolute left-4 w-1.5 h-1.5 rounded-full bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,0.5)]" />}
                       <span className={cn("font-medium", winner === 'a' ? "text-white" : "text-slate-400")}>{row.render(domainA)}</span>
                    </div>
                    <div className="col-span-4 p-6 flex items-center justify-center relative">
                       ...
                       <span className={cn("font-medium", winner === 'b' ? "text-white" : "text-slate-400")}>{row.render(domainB)}</span>
                    </div>
                  </div>
                );
              })}
                 ...
                 <div className="p-8 rounded-2xl bg-cyan-400/5 border border-cyan-400/20 text-center relative overflow-hidden">
                    ...
                    <div className="mt-6 pt-6 border-t border-cyan-400/10 flex justify-center gap-12">
                       <div className="text-center">
                          <span className="block text-2xl font-bold text-white">{tally.a}</span>
                          <span className="text-[9px] font-mono text-slate-500 uppercase">CATEGORY WINS</span>
                       </div>
                       <div className="text-center">
                          <span className="block text-2xl font-bold text-white">{tally.b}</span>
                          <span className="text-[9px] font-mono text-slate-500 uppercase">CATEGORY WINS</span>
                       </div>
                    </div>
                 </div>
        ...
        {/* Footer Actions */}
        <div className="p-6 sm:p-8 bg-[#020408] border-t border-white/10 flex items-center justify-between shrink-0">
           <button onClick={onClose} className="text-[10px] font-mono text-slate-500 hover:text-white uppercase tracking-widest">
              Close Comparison
           </button>
           <div className="flex gap-4">
              <button className="px-6 py-2.5 bg-white/5 text-white font-bold uppercase text-[10px] tracking-widest rounded-lg border border-white/10 hover:bg-white/10 transition-colors">
                Compare Others
              </button>
