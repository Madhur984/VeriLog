  ...
  return (
    <BlueprintContainer>
      ...
      <div className="w-full max-w-5xl flex flex-col items-center gap-16">
        {/* Toggle Interaction */}
        <div className="flex items-center gap-10 bg-black/40 p-4 rounded-full border border-white/5">
            <button 
                onClick={() => setIsAsic(false)}
                className={`px-10 py-4 rounded-full font-black uppercase tracking-widest text-[10px] transition-all ${!isAsic ? 'bg-[#00D4FF] text-black shadow-cyan-glow' : 'opacity-30 hover:opacity-100'}`}
            >
                FPGA Path
            </button>
            ...
            <button 
                ...
                className={`px-10 py-4 rounded-full font-black uppercase tracking-widest text-[10px] transition-all ${isAsic ? 'bg-burnished-copper text-white shadow-lg' : 'opacity-30 hover:opacity-100'}`}
            >
                ASIC Path
            </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-20 items-stretch w-full">
            ...
            <div className="relative group">
                <AnimatePresence mode="wait">
                    <motion.div 
                        ...
                        className={`h-full p-12 rounded-[60px] border relative overflow-hidden flex flex-col items-center justify-center text-center space-y-8 ${isAsic ? 'bg-burnished-copper/5 border-burnished-copper/30' : 'bg-plasma-cyan/5 border-plasma-cyan/30'}`}
                    >
            ...
            <div className="flex flex-col gap-6 justify-center">
                {[
                    ...
                    <div key={i} className="p-8 rounded-[40px] bg-white/[0.02] border border-white/5 flex items-center justify-between group hover:border-white/20 transition-all">
                        ...
                        <span className={`text-xl font-black italic tracking-tight ${stat.color}`}>{stat.val}</span>
                    </div>
                ))}
            </div>
        </div>
