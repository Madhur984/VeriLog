...
export const S07b_ClockSignal: React.FC<{ isActive: boolean }> = ({ isActive }) => {
  const [freq, setFreq] = React.useState(4.2);
  const duration = 2.4 / freq; // Scale duration based on frequency for animations
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-24 items-start w-full">
        ...
        <div className="space-y-10 sticky top-24">
          ...
          <div className="space-y-8 max-w-xl">
            ...
            <div className="p-8 pb-10 rounded-[40px] bg-white/[0.02] border border-white/5 border-l-4 border-l-plasma-cyan group shadow-xl">
                 <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-4">
                        <Radio size={20} className="text-plasma-cyan group-hover:scale-110 transition-transform" />
                        <span className="micro-text uppercase tracking-widest text-white/60 font-black">Oscillation Tuning</span>
                    </div>
                    <span className="mono-text text-plasma-cyan font-black text-xs">{freq.toFixed(2)} GHz</span>
                 </div>
                 <input 
                    type="range" 
                    min="1" 
                    ...
                    className="w-full accent-plasma-cyan bg-white/5 h-1.5 rounded-full appearance-none cursor-pointer"
                 />
                 <div className="flex justify-between mt-4 micro-text text-[8px] opacity-30 font-black tracking-widest">
                    <span>1.0 GHz</span>
                    <span>NOMINAL</span>
                    <span>8.0 GHz OVERCLOCK</span>
                 </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-16 relative">
                <div className="relative w-full h-64 bg-white/[0.02] border border-white/10 rounded-[40px] overflow-hidden flex items-center justify-center">
                    ...
                    <div className="absolute inset-0 opacity-10 flex items-center justify-center">
                        <svg width="100%" height="100%" viewBox="0 0 800 200">
                             <motion.path 
                                ...
                                d="M -100 100 Q -75 25 -50 100 T 0 100 T 50 100 T 100 100 T 150 100 T 200 100 T 250 100 T 300 100 T 350 100 T 400 100 T 450 100 T 500 100 T 550 100 T 600 100 T 650 100 T 700 100 T 750 100 T 800 100" 
                                stroke="white" 
                                strokeWidth="1" 
                                fill="none" 
                             />
                    ...
                    <div className="relative z-10 flex gap-4 items-end h-32">
                        {[...Array(24)].map((_, i) => (
                             <motion.div 
                                ...
                                transition={{ duration: duration * 0.25, repeat: Infinity, delay: i * (0.05 / (freq/4)), ease: "easeInOut" }}
                                className="w-2 bg-plasma-cyan/30 rounded-full"
                                style={{ backgroundColor: `rgba(0, 212, 255, ${0.1 + (i / 24) * 0.5})` }}
                             />
                        ))}
                    </div>
                    ...
                    <div className="absolute top-6 right-8 flex flex-col items-end">
                        ...
                        <div className="hero-text text-2xl text-white">{freq.toFixed(2)} GHz</div>
                    </div>
                </div>

                <div className="flex flex-col items-center gap-10">
                    <div className="relative">
                        <div className="w-32 h-32 rounded-[40px] bg-plasma-cyan/10 border-2 border-plasma-cyan flex items-center justify-center text-plasma-cyan shadow-cyan-glow">
                             <motion.div
                    ...
                    <div className="grid grid-cols-2 gap-8 w-full max-w-md">
                        <div className="p-8 rounded-[35px] bg-[#0A0A0B] border border-white/5 text-center space-y-2">
                             ...
                             <div className="hero-text text-xl text-white">±{(0.01 + (freq * 0.002)).toFixed(3)}ps</div>
                        </div>
                        <div className="p-8 rounded-[35px] bg-[#0A0A0B] border border-white/5 text-center space-y-2">
            ...
            <div className="mt-8 flex items-center justify-between p-6 bg-bg-elev border border-border-soft rounded-3xl shadow-neo">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="body-text text-[10px] opacity-30 italic">{freq > 5 ? 'STAIR_STEP_ALERT: Clock Skew risk increasing...' : 'Verifying synchronous pulse propagation delay... OK'}</div>
                    </div>
                </div>
                <div className="flex gap-2 items-center">
                    <div className="w-2 h-2 rounded-full bg-plasma-cyan animate-pulse shadow-[0_0_8px_#00D4FF]" />
                    <span className="micro-text text-[10px] text-plasma-cyan font-black uppercase tracking-widest">{freq > 5 ? 'STRESSED' : 'LOCKED'}</span>
                </div>
            </div>
        </div>
      </div>
