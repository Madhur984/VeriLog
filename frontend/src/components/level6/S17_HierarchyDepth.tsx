  ...
  return (
    <BlueprintContainer>
      ...
      <div className="w-full max-w-4xl flex flex-col items-center gap-16 relative">
          ...
          <div className="relative w-full h-[500px] flex items-center justify-center p-12 bg-black/40 border border-white/5 rounded-[60px] overflow-hidden">
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        {/* Current Level Node */}
                        <div className="p-12 rounded-[50px] bg-plasma-cyan/10 border border-plasma-cyan/30 flex flex-col items-center gap-6 shadow-cyan-glow relative">
                             <div className="w-24 h-24 rounded-3xl bg-black flex items-center justify-center text-plasma-cyan shadow-inner">
                                {depth === 0 ? <Cpu size={48} /> : <HardDrive size={48} />}
                             </div>
                             <div className="text-center">
                                <h3 className="text-3xl font-black italic tracking-tighter uppercase whitespace-nowrap">
                                    {depth === 0 ? "CPU Architecture" : depth === 1 ? "Arithmetic Logic Unit" : "Arithmetic Core"}
                                </h3>
                                <div className="text-[10px] font-black uppercase tracking-widest text-[#00D4FF] opacity-40 mt-1">
                        ...
                        <div className="flex gap-10">
                            {[1, 2].map((i) => (
                                <div key={i} className="p-6 rounded-3xl border border-white/5 bg-white/[0.02] flex items-center gap-4 opacity-40 grayscale blur-[1px]">
                                    <Layers size={16} />
                                    <span className="text-[10px] uppercase font-black tracking-widest">Sub-Component {i}</span>
                                </div>
                            ))}
                        </div>
                    </motion.div>
          ...
          <div className="flex items-center gap-8 bg-black/60 p-6 rounded-full border border-white/10 backdrop-blur-xl shadow-2xl">
                <button 
                    ...
                    className="p-5 rounded-full bg-white/5 border border-white/10 text-plasma-cyan disabled:opacity-10 transition-all hover:bg-plasma-cyan hover:text-black"
                >
                    <Search className="scale-x-[-1]" size={28} />
                </button>
                <div className="flex flex-col items-center gap-1">
                    <div className="text-[10px] font-black uppercase tracking-[0.4em] opacity-30">Depth Scan</div>
                    <div className="text-2xl font-mono font-black italic text-plasma-cyan">L.0{depth + 1}</div>
                </div>
                <button 
                    disabled={depth === 2}
