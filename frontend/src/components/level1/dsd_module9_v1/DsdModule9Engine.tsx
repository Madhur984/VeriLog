...
import { S01_Deck }          from './scenes/S01_Deck';
import { S02_Combinational } from './scenes/S02_Combinational';
import { S03_Sequential }    from './scenes/S03_Sequential';
import { S04_Adders }        from './scenes/S04_Adders';
import { S05_Boss }          from './scenes/S05_Boss';
import { S06_Recap }         from './scenes/S06_Recap';
...
const PAGES: Page[] = [
  { id: 'cover',         part: 'PART I · THE RECALL DECK',     label: 'Recall & Prove',       subtitle: 'Cover · why review is where it sticks.',           Component: S00_Cover },
  { id: 'deck',          part: 'PART I · THE RECALL DECK',     label: 'The Recall Deck',      subtitle: 'Flip · analogy on the front, real logic on the back.', Component: S01_Deck },
  { id: 'combinational', part: 'PART II · THE DRILLS',         label: 'Combinational Drill',  subtitle: 'No memory · MUX, decoders and gates.',             Component: S02_Combinational },
  { id: 'sequential',    part: 'PART II · THE DRILLS',         label: 'Sequential Drill',     subtitle: 'Memory · latches, flip-flops and the clock.',      Component: S03_Sequential },
  { id: 'adders',        part: 'PART II · THE DRILLS',         label: 'Adders Drill',         subtitle: 'Half, full and the ripple carry.',                 Component: S04_Adders },
  { id: 'boss',          part: 'PART III · THE PROVING GROUND', label: 'Mixed Boss Round',     subtitle: 'All three topics, shuffled and harder.',           Component: S05_Boss },
  { id: 'recap',         part: 'PART III · THE PROVING GROUND', label: 'The Cheatsheet',       subtitle: 'One page · share the deck, then build.',           Component: S06_Recap },
];
...
const getPartTheme = (part: string) => {
  ...
  if (part.startsWith('PART I '))   return { primary: '#fb7185', secondary: '#fda4af', glow: 'rgba(251, 113, 133, 0.12)' };
  return { primary: '#fb7185', secondary: '#fda4af', glow: 'rgba(251, 113, 133, 0.12)' };
};
...
export const DsdModule9Engine: React.FC<{
  isDarkMode: boolean;
  onThemeToggle: () => void;
  ...
  useEffect(() => {
    if (initialChapter) {
      ...
      // No chapter param → user explicitly wants /dsd/9, start from cover
  ...
  return (
    <div className={`flex h-screen overflow-hidden transition-colors duration-700 relative ${isDarkMode ? 'bg-[#020100]' : 'bg-white'}`}>
        ...
        >
          ...
          <div className="hidden md:flex items-center gap-8">
            <div className="text-right">
              <div className="text-[8px] font-mono uppercase tracking-widest opacity-30">DSD MODULE 09</div>
      ...
      {done && (
        <ModuleComplete
          isDark={isDarkMode}
          moduleTitle={MODULE_LABELS['dsd/9'] ?? 'this module'}
          accent={theme.primary}
          topics={Array.from(new Set(PAGES.map((p) => p.label)))}
          onPortal={() => navigate('/portal')}
          next={{ label: MODULE_LABELS['dsd/10'] ?? 'Next module', onGo: () => navigate('/dsd/10') }}
        />
      )}
    </div>
  );
