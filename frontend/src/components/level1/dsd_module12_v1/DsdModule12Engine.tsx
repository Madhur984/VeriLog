...
import { Network, Moon, Sun, ArrowRight, ArrowLeft } from 'lucide-react';
...
const PAGES: Page[] = [
  ...
  { id: 'circuit',    part: 'PART II · THE MECHANISM', label: 'The Full Circuit',    subtitle: 'Gate-level schematic, the prefix tree wired live.', Component: S08_Circuit },
  ...
  { id: 'recap',      part: 'PART III · THE PAYOFF',   label: 'Recap & Verilog',       subtitle: 'One page · Kogge-Stone code, what next.',          Component: S07_Recap },
];

const getPartTheme = (part: string) => {
  if (part.startsWith('PART III ')) return { primary: '#fb7185', secondary: '#fda4af', glow: 'rgba(251, 113, 133, 0.12)' };
  if (part.startsWith('PART II '))  return { primary: '#34d399', secondary: '#6ee7b7', glow: 'rgba(52, 211, 153, 0.12)' };
  if (part.startsWith('PART I '))   return { primary: '#818cf8', secondary: '#a5b4fc', glow: 'rgba(129, 140, 248, 0.12)' };
  return { primary: '#818cf8', secondary: '#a5b4fc', glow: 'rgba(129, 140, 248, 0.12)' };
};
    ...
    <div className={`w-[320px] h-full flex-shrink-0 border-r-2 flex flex-col z-20 transition-all duration-700 relative ${isDarkMode ? 'bg-[#040200]' : 'bg-slate-50'}`} style={{ borderColor }}>
      <header className="p-10 border-b" style={{ borderColor }}>
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-2xl bg-indigo-400 flex items-center justify-center text-black"><Network size={20} /></div>
          <div>
            <h2 className={`text-sm font-black tracking-tight ${textColor}`}>Digital System Design</h2>
            <p className="text-[10px] uppercase font-mono tracking-widest font-bold transition-colors duration-500" style={{ color: theme.primary }}>Module 12 · Parallel Prefix Adder</p>
          </div>
        </div>
      </header>
...
export const DsdModule12Engine: React.FC<{ isDarkMode: boolean; onThemeToggle: () => void; initialChapter?: string; }> = ({ isDarkMode, onThemeToggle, initialChapter }) => {
  ...
  useEffect(() => {
    ...
    if (id) { const target = `/dsd/12/${id}`; if (window.location.pathname !== target) navigate(target, { replace: true }); }
  }, [current, navigate]);
        ...
        <header className="h-16 lg:h-20 border-b flex items-center justify-between px-4 lg:px-12 z-10 gap-3" style={{ borderColor: 'var(--border-soft)' }}>
          ...
          <div className="hidden md:flex items-center gap-8">
            <div className="text-right">
              <div className="text-[8px] font-mono uppercase tracking-widest opacity-30">DSD MODULE 12</div>
