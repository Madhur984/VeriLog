    ...
    return (
        <div className="flex flex-col gap-32 w-full max-w-none mx-auto">
            ...
            <div className="grid grid-cols-1 2xl:grid-cols-2 gap-20">
                ...
                <div className="space-y-12">
                    <div className={`p-16 rounded-[6rem] space-y-12 shadow-2xl relative overflow-hidden ${isDarkMode ? 'bg-[#0A0C10] border border-white/5' : 'bg-slate-50 border border-slate-200'}`}>
                        <div className="flex justify-between items-start relative z-10">
                            <div className="flex items-center gap-6">
                                <div className={`p-6 rounded-[2rem] transition-all duration-700 ${state.dither ? 'bg-orange-500 text-black shadow-lg shadow-orange-500/20' : (isDarkMode ? 'bg-white/5 text-white/20' : 'bg-slate-100 text-slate-400')}`}>
                                    <Dna size={40} />
                                </div>
                                <div className="flex flex-col">
                                    <h4 className={`text-4xl font-black italic uppercase tracking-tighter ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Quantization</h4>
                                    <span className={`text-[10px] font-mono uppercase tracking-widest ${isDarkMode ? 'text-white/20' : 'text-slate-400'}`}>Discrete Domain Mapping</span>
                                </div>
                            </div>
                        </div>

                        <p className={`text-xl font-medium leading-relaxed italic relative z-10 ${isDarkMode ? 'text-white/40' : 'text-slate-600'}`}>
                        ...
                        <div className={`grid grid-cols-2 gap-8 pt-12 border-t relative z-10 ${isDarkMode ? 'border-white/5' : 'border-slate-200'}`}>
                             <div className="space-y-3">
                                <div className={`text-[9px] font-black uppercase tracking-widest font-mono ${isDarkMode ? 'text-white/20' : 'text-slate-400'}`}>Signal-to-Noise</div>
                             ...
                             <div className="space-y-3">
                                <div className={`text-[9px] font-black uppercase tracking-widest font-mono ${isDarkMode ? 'text-white/20' : 'text-slate-400'}`}>Total_Resolution</div>
