...
export const CONTENT = ({
  ...
  scenes: [
    {
      ...
      theoryEN: [
        ...
        'Once every minterm is sitting on its own wire, you get a bonus superpower: OR together the output lines for the minterms where your function is 1 and you have built any Boolean function in sum-of-products form. One decoder plus one OR gate per output builds real logic - we will literally build XOR this way (XOR = Y1 + Y2).',
    ...
    },
    {
      ...
      theoryEN: [
        ...
        'Why bother with an enable at all? Two big reasons. First, you can disable a chip when it is not being addressed, which matters when many chips share a bus and only one should respond at a time. Second, and more cleverly, the enable lets you cascade decoders: feed the high address bit and its complement into the enables of two smaller decoders and you have built the next size up, a 3-to-8 from two 2-to-4s.',
    ...
    },
    {
      ...
      theoryEN: [
        ...
        'The third proof is the key idea: the canonical-form theorem that a decoder plus OR gates builds any function. Any F is the sum of its on-set minterms, the decoder supplies every minterm on a wire, so F is the OR of the selected outputs. The worked example builds the full-adder SUM = OR(D1,D2,D4,D7) on a 3-to-8 decoder.',
      ...
      theoryHI: [
        ...
        'तीसरा proof सबसे अहम है: canonical-form theorem कि एक decoder जमा OR gates कोई भी function बना देते हैं। कोई भी F अपने on-set minterms का योग है, decoder हर minterm एक wire पर देता है, तो F चुने outputs का OR है। worked example एक 3-to-8 decoder पर full-adder SUM = OR(D1,D2,D4,D7) बनाता है।',
      ...
      transcriptEN:
        'Deriving the equations beats memorising them. The first proof demands each output be HIGH for exactly one combination, writes it as a product of literals, and reads off D0 equals A prime B prime through D3 equals A B; then it checks that the four sum to one, proving exactly one is HIGH. The second proof ANDs an enable E into every output, so Di equals E times the minterm; E zero silences all, E one decodes normally, and the same AND cascades two two-to-fours into a three-to-eight. The third proof is the key idea: any function is the OR of its on-set minterms, the decoder supplies every minterm on a wire, so a decoder plus an OR gate builds any function - the worked example builds the full-adder SUM from minterms one, two, four, seven on a three-to-eight decoder. Step through all three.',
      transcriptHI:
        'equations derive करना उन्हें रटने से बेहतर है। पहला proof माँगता है कि हर output ठीक एक combination के लिए HIGH हो, इसे literals के product के रूप में लिखता है, और पढ़ता है D0 बराबर A prime B prime से D3 बराबर A B तक; फिर जाँचता है कि चारों का योग एक है, जो साबित करता है ठीक एक HIGH है। दूसरा proof एक enable E को हर output में AND करता है, तो Di बराबर E गुणा minterm; E शून्य सब चुप करता है, E एक सामान्य decode करता है, और वही AND दो two-to-fours को एक three-to-eight में cascade करता है। तीसरा proof सबसे अहम है: कोई भी function अपने on-set minterms का OR है, decoder हर minterm एक wire पर देता है, तो एक decoder जमा एक OR gate कोई भी function बना देते हैं - worked example एक three-to-eight decoder पर minterms एक, दो, चार, सात से full-adder SUM बनाता है। तीनों से step कीजिए।',
      visualNote:
  ...
  flashcards: [
    ...
    },
    {
      ...
      backEN:
        'It exposes every minterm on its own wire, so OR the minterm lines where F = 1 to build any SOP function. Example: XOR = Y1 + Y2 = A prime.B + A.B prime.',
