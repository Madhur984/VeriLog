...
import { Boxes, Moon, Sun, ArrowRight, ArrowLeft } from 'lucide-react';
...
import { S01_Interface }    from './scenes/S01_Interface';
import { S02_Video }        from './scenes/S02_Video';
import { S03_Sum }          from './scenes/S03_Sum';
import { S04_Carry }        from './scenes/S04_Carry';
import { S05_TruthTable }   from './scenes/S05_TruthTable';
import { S06_Evolution }    from './scenes/S06_Evolution';
import { S07_Architecture } from './scenes/S07_Architecture';
import { S08_Practice }     from './scenes/S08_Practice';
import { S09_Build }        from './scenes/S09_Build';
    ...
    >
      <header className="p-10 border-b" style={{ borderColor }}>
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-2xl bg-violet-400 flex items-center justify-center text-black">
            <Boxes size={20} />
          </div>
          <div>
...
export const DsdModule8Engine: React.FC<{
  isDarkMode: boolean;
  onThemeToggle: () => void;
  ...
  useEffect(() => {
    if (initialChapter) {
      ...
      // No chapter param → user explicitly wants /dsd/8, start from cover
  ...
  return (
    <div className={`flex h-screen overflow-hidden transition-colors duration-700 relative ${isDarkMode ? 'bg-[#020100]' : 'bg-white'}`}>
        ...
        >
          ...
          <div className="hidden md:flex items-center gap-8">
            <div className="text-right">
              <div className="text-[8px] font-mono uppercase tracking-widest opacity-30">DSD MODULE 08</div>
      ...
      {done && (
        <ModuleComplete
          isDark={isDarkMode}
          moduleTitle={MODULE_LABELS['dsd/8'] ?? 'this module'}
          accent={theme.primary}
          topics={Array.from(new Set(PAGES.map((p) => p.label)))}
          onPortal={() => navigate('/portal')}
          next={{ label: MODULE_LABELS['dsd/9'] ?? 'Next module', onGo: () => navigate('/dsd/9') }}
        />
      )}
    </div>
  );
