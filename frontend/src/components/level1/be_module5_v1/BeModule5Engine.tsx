...
import { S02_Baseline }    from './scenes/S02_Baseline';
import { S03_ZenerVI }     from './scenes/S03_ZenerVI';
import { S04_ZenerReg }    from './scenes/S04_ZenerReg';
import { S05_LedEL }       from './scenes/S05_LedEL';
import { S06_LedSpectrum } from './scenes/S06_LedSpectrum';
import { S07_PhotoCircuit }from './scenes/S07_PhotoCircuit';
import { S08_PhotoResp }   from './scenes/S08_PhotoResp';
import { S09_Matrix }      from './scenes/S09_Matrix';
import { S10_Quiz }        from './scenes/S10_Quiz';
...
const PAGES: Page[] = [
  { id: 'cover',     step: 'Open',     label: 'The Special-Purpose Diode Gala', subtitle: 'Zener · LED · Photodiode — three VIPs.',     Component: S00_Cover       },
  { id: 'video',     step: 'Lecture',  label: 'Video · EN + HI commentary',     subtitle: 'Curator-style walkthrough in two languages.', Component: S01_Video       },
  { id: 'baseline',  step: 'Step 1',   label: 'Baseline vs The Specialists',    subtitle: 'P-N as a switch · 3 elite behaviours.',       Component: S02_Baseline    },
  { id: 'zener-vi',  step: 'Zener · 1',label: 'Zener V-I & Breakdown',          subtitle: 'Zener vs Avalanche · V_Z, I_ZM.',             Component: S03_ZenerVI     },
  { id: 'zener-reg', step: 'Zener · 2',label: 'Voltage Regulator Circuit',      subtitle: 'Bodyguard absorbs ΔI_Z to hold V_Z.',         Component: S04_ZenerReg    },
  { id: 'led-el',    step: 'LED · 1',  label: 'Electroluminescence · LED',      subtitle: 'Electron-hole recombination → photon.',       Component: S05_LedEL       },
...
const ACCENTS = ['#facc15', '#fb923c', '#22d3ee', '#ef4444', '#f87171', '#fbbf24', '#a78bfa', '#22c55e', '#10b981', '#e879f9', '#34d399'];
const themeFor = (idx: number) => {
  const c = ACCENTS[idx] ?? '#facc15';
    ...
    >
      <header className="p-8 border-b" style={{ borderColor }}>
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-black" style={{ background: '#facc15' }}>
            <Sparkles size={20} />
          </div>
          <div>
            ...
            <p className="text-[10px] uppercase font-mono tracking-widest font-bold transition-colors duration-500" style={{ color: theme.primary }}>
              Module 05 · Special-Purpose Diodes
            </p>
          </div>
        </div>
      </header>
      ...
      <nav className="p-6 flex-1 overflow-y-auto space-y-1">
        {PAGES.map((page, idx) => {
          ...
          const accent = ACCENTS[idx] ?? '#facc15';
          return (
            <button
              key={page.id}
...
export const BeModule5Engine: React.FC<{
  isDarkMode: boolean;
  onThemeToggle: () => void;
  ...
  useEffect(() => {
    ...
    if (id) {
      const target = `/basic-electronics/5/${id}`;
  ...
  return (
    <div className={`flex h-screen overflow-hidden transition-colors duration-700 relative ${isDarkMode ? 'bg-[#01040c]' : 'bg-white'}`}>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative z-10">
        ...
        <div ref={contentRef} className="flex-1 overflow-y-auto scroll-smooth">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="max-w-7xl mx-auto px-12 py-14"
            >
        ...
        <footer
          className="h-20 border-t flex items-center justify-between px-12 z-10"
          style={{ borderColor: isDarkMode ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)' }}
        >
          <button
            disabled={current === 0}
          ...
          <button
            ...
            className={`flex items-center gap-3 px-8 py-2.5 rounded-xl font-black text-black transition-all duration-500 active:scale-95 ${
