...
export const CONTENT = ({
  ...
  "scenes": [
    ...
    },
    {
      ...
      "kind": "truth",
      "subtitle": "The four mornings compressed into one precise table.",
      "theoryEN": [
        "Now we drop the story for a moment and write those same four mornings as a precise truth table, the engineer's standard tool. A truth table lists every possible input on the left and the matching outputs on the right, so two input columns (x and y) and two output columns (D and B) capture the entire behaviour with no story-telling left to argue about.",
    ...
    },
    {
      ...
      "theoryEN": [
        "Now the famous second output, the Borrow, and the one place where students slip. Its law is B = (NOT x) AND y, written x'.y, where x' (read 'x-prime') means NOT-x, the inverted value of x. The key detail is that the NOT sits on x, the spaces, not on y - get that backwards and the whole circuit is wrong.",
