          ...
          <div key={i} className="flex flex-col items-center gap-1.5 sm:gap-2 min-w-0">
            ...
            <motion.button
              onClick={() => {
                  ...
                  playBitTone(7-i, bits[i] === 0 ? 'high' : 'low');
      ...
      <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2">
        {[
          ...
          <div key={i} className={`p-3 rounded-xl text-center ${isDarkMode ? 'bg-white/5' : 'bg-white border border-gray-100'}`}>
            <div className="font-mono text-sky-400 text-xs font-black">{ex.label}_2</div>
        ...
        <div className="grid grid-cols-4 md:grid-cols-8 gap-3 sm:gap-4">
          {weights8.map((w, i) => (
            ...
            >
              ...
              <span className="font-mono text-[11px] opacity-40 uppercase">2^{7 - i}</span>
            </motion.div>
          ))}
        </div>
          ...
          <div className="flex justify-between gap-2 mb-8">
            {['1', '1', '0', '1'].map((bit, i) => (
              <div key={i} className="flex flex-col items-center">
                ...
                <span className="mt-2 font-mono text-[11px] font-bold opacity-60">* {Math.pow(2, 3 - i)}</span>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className={`p-5 sm:p-6 rounded-3xl border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-200'}`}>
            ...
            <div className={`font-mono text-xs leading-loose p-4 rounded-xl overflow-x-auto ${isDarkMode ? 'bg-black/40' : 'bg-white border border-gray-100'}`}>
              ...
              <div>2^-3 = <span className="text-sky-400">0.125</span></div>
              <div className="mt-3 opacity-60">Example:</div>
              <div>(0.101)_2 = 0.5 + 0.125 = <span className="text-sky-400 font-black">0.625</span></div>
              <div>(11.001)_2 = 2+1+0.125 = <span className="text-sky-400 font-black">3.125</span></div>
            </div>
          </div>
        </div>
      </div>
