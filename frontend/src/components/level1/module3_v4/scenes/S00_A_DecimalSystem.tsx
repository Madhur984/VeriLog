  ...
  return (
    <div className="max-w-5xl mx-auto space-y-16 py-12">
      ...
      <motion.div
        ...
        className={`p-5 sm:p-8 rounded-3xl border text-center ${cardBg}`}
      >
        <p className={`font-mono text-xs uppercase tracking-widest mb-4 ${subTextColor}`}>General Positional Notation</p>
        <div className={`text-sm sm:text-xl md:text-2xl font-black font-mono break-words ${textColor}`}>
      ...
      <div className={`p-5 sm:p-8 rounded-[2rem] border ${cardBg}`}>
        ...
        <div className="flex justify-center gap-3 sm:gap-4 flex-wrap mb-8">
          {EXAMPLE.map((d, i) => (
            ...
            >
              <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-2xl border-2 flex items-center justify-center text-2xl sm:text-3xl font-black transition-all
        ...
        <div className={`text-center font-mono text-xs sm:text-sm p-4 rounded-xl break-words ${isDarkMode ? 'bg-black/40' : 'bg-amber-50/50 border border-amber-100'}`}>
      ...
      <div className={`p-5 sm:p-8 rounded-[2rem] border ${cardBg}`}>
        ...
        <div className="flex justify-center gap-3 sm:gap-6 flex-wrap mb-8">
          {digits.map((d, i) => (
            <div key={i} className="flex flex-col items-center gap-3">
              ...
              <div className="flex flex-col items-center gap-1">
                <button
                  ...
                  className={`text-lg font-black w-10 h-10 sm:h-8 rounded-lg transition-all ${isDarkMode ? 'hover:bg-amber-500/20 text-white/40 hover:text-amber-400' : 'hover:bg-amber-50 text-gray-400 hover:text-amber-600'}`}
                >[^]</button>
        ...
        <div className={`text-center p-4 sm:p-6 rounded-2xl ${isDarkMode ? 'bg-amber-500/10 border border-amber-500/20' : 'bg-amber-50 border border-amber-200'}`}>
          <p className={`font-mono text-xs uppercase tracking-widest opacity-60 mb-2 ${textColor}`}>Total Value</p>
          <div className="text-4xl sm:text-5xl font-black text-amber-500 break-words">{total}</div>
          <p className={`font-mono text-[10px] mt-2 opacity-40 break-words ${textColor}`}>
      ...
      <div className={`p-5 sm:p-8 rounded-3xl border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-200'}`}>
        ...
        <div className={`font-mono text-sm leading-loose ${textColor}`}>
          ...
          <div className={`p-4 rounded-xl text-sm sm:text-base break-words ${isDarkMode ? 'bg-black/40' : 'bg-white border border-gray-100'}`}>
