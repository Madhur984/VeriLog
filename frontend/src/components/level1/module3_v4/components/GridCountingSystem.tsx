...
export const GridCountingSystem: React.FC<Props> = ({ 
    ...
    return (
        <div className="flex flex-col items-center space-y-8 py-8">
            ...
            <div className={`grid grid-cols-10 gap-1 p-2 rounded-2xl border ${isDarkMode ? 'bg-black/40 border-white/5' : 'bg-white border-gray-100 shadow-xl'}`}>
                {numbers.map((num, i) => (
                    <motion.div
                        key={i}
                        ...
                        className={`
                            w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-md text-[10px] md:text-xs font-mono font-bold
            ...
            {highlightIllegal && (
                <div className={`flex items-center gap-4 px-6 py-3 rounded-full border ${isDarkMode ? 'bg-red-500/10 border-red-500/20' : 'bg-red-50 border-red-100'}`}>
