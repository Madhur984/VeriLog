...
import { S02_GoodBoyHostel }    from './scenes/S02_GoodBoyHostel';
import { S03_HostelVsTapri }    from './scenes/S03_HostelVsTapri';
import { S04_BreakingBonds }    from './scenes/S04_BreakingBonds';
import { S05_DopingJugaad }     from './scenes/S05_DopingJugaad';
import { S06_NTypeSquad }       from './scenes/S06_NTypeSquad';
import { S07_PTypeSquad }       from './scenes/S07_PTypeSquad';
import { S08_FlowDuel }         from './scenes/S08_FlowDuel';
import { S09_CheatSheet }       from './scenes/S09_CheatSheet';
import { S10_NeutralityHeat }   from './scenes/S10_NeutralityHeat';
...
const PAGES: Page[] = [
  { id: 'cover',     part: 'PART I · INTRINSIC LAB',    label: 'Madhur\'s Silicon Lab',         subtitle: 'Cover · welcome to the Tapri.',                            Component: S00_Cover },
  { id: 'video',     part: 'PART I · INTRINSIC LAB',    label: 'Semiconductor Physics Decoded', subtitle: 'Lecture · the Madhur way.',                                Component: S01_VideoLecture },
  { id: 'hostel',    part: 'PART I · INTRINSIC LAB',    label: 'The Good Boy Hostel',           subtitle: 'Intrinsic Silicon · the disciplined 4-seater.',           Component: S02_GoodBoyHostel },
  { id: 'tapri',     part: 'PART I · INTRINSIC LAB',    label: 'Hostel vs Tapri',               subtitle: 'Energy gap · valence vs conduction band.',                Component: S03_HostelVsTapri },
  { id: 'jump',      part: 'PART I · INTRINSIC LAB',    label: 'Breaking Covalent Bonds',       subtitle: 'Heat · light · the trigger to roam free.',                Component: S04_BreakingBonds },
  { id: 'jugaad',    part: 'PART II · DOPING JUGAAD',   label: 'Pure Si Is Too Slow',           subtitle: '1 free electron per 10¹² atoms - we need a hack.',       Component: S05_DopingJugaad },
  { id: 'ntype',     part: 'PART II · DOPING JUGAAD',   label: 'N-Type · 5-Friend Squad',       subtitle: 'Pentavalent dopant · donor atom · awara electron.',        Component: S06_NTypeSquad },
  { id: 'ptype',     part: 'PART II · DOPING JUGAAD',   label: 'P-Type · 3-Friend Squad',       subtitle: 'Trivalent dopant · acceptor atom · the hole.',            Component: S07_PTypeSquad },
  { id: 'flow',      part: 'PART II · DOPING JUGAAD',   label: 'Electron vs Hole Flow',         subtitle: 'Musical chairs · conventional current.',                  Component: S08_FlowDuel },
  { id: 'cheat',     part: 'PART II · DOPING JUGAAD',   label: 'N vs P · Cheat Sheet',          subtitle: 'Snapshot for tomorrow\'s mid-term.',                        Component: S09_CheatSheet },
  { id: 'neutral',   part: 'PART II · DOPING JUGAAD',   label: 'Neutrality + Heat Twist',       subtitle: 'Doped material is still neutral · negative temp coefficient.', Component: S10_NeutralityHeat },
  { id: 'practice',  part: 'PART III · ASSESSMENT',     label: 'Practice Arena',                subtitle: 'Drills · sandbox · sprint.',                              Component: S11_PracticeArena },
];

const getPartTheme = (part: string) => {
  if (part.startsWith('PART III '))   return { primary: '#f43f5e', secondary: '#fb7185', glow: 'rgba(244, 63, 94, 0.12)' };
  if (part.startsWith('PART II '))    return { primary: '#a78bfa', secondary: '#c4b5fd', glow: 'rgba(167, 139, 250, 0.12)' };
  if (part.startsWith('PART I '))     return { primary: '#fb923c', secondary: '#fdba74', glow: 'rgba(251, 146, 60, 0.14)' };
  return { primary: '#fb923c', secondary: '#fdba74', glow: 'rgba(251, 146, 60, 0.14)' };
};
  ...
  return (
    <div
      className={`w-[320px] h-full flex-shrink-0 border-r flex flex-col z-20 transition-all duration-700 relative ${
        isDarkMode ? 'bg-[#0a0805]/60 backdrop-blur-md' : 'bg-slate-50/40 backdrop-blur-md'
      }`}
      style={{ borderColor }}
    >
...
export const BeModule2Engine: React.FC<{
  isDarkMode: boolean;
  onThemeToggle: () => void;
  ...
  useEffect(() => {
    ...
    if (id) {
      const target = `/basic-electronics/2/${id}`;
