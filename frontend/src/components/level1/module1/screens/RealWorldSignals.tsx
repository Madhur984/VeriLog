...
export const RealWorldSignals: React.FC<ScreenProps> = ({ 
  ...
  return (
    <div className="section-content relative overflow-hidden" {...focusProps}>
      ...
      <div className="mb-12 space-y-4 text-left">
        <h2 className="text-[var(--accent-primary)] font-mono text-[10px] uppercase tracking-[0.5em] opacity-40">Discovery</h2>
        <h1 className="title-xl italic">SIGNALS ARE EVERYWHERE.</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl w-full relative">
        <AnimatePresence mode="wait">
            ...
            >
                <Eye className="text-[var(--accent-primary)] w-10 h-10 mb-4 animate-pulse" />
                <h3 className="text-[10px] font-mono uppercase tracking-[0.3em] mb-2">Pattern Recognition</h3>
                <p className="body text-white/50 text-[10px] max-w-xs mb-6">Does a wireless signal follow the same physical laws as a voice wave? Prepare to bridge the domains.</p>
                <VeriButton 
                    variant="signal"
                    onClick={() => {
        ...
        {items.map((item, idx) => (
          <motion.div
            ...
            whileHover={!predictionMode ? { scale: 1.02, backgroundColor: 'rgba(255,255,255,0.03)' } : {}}
            onClick={() => {
              if (predictionMode) return;
              triggerHaptic?.('light');
            }}
            style={getDimStyle(false)}
            className="group relative p-6 glass-card cursor-pointer overflow-hidden border-white/5 hover:border-[var(--accent-primary)]/20"
          >
            ...
            <div className="flex flex-col gap-4 relative z-10 text-left">
              <div className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:border-[var(--accent-primary)]/30 transition-all">
                <item.icon size={20} style={{ color: item.color }} />
              </div>
              <div className="space-y-1">
                <h3 className="text-lg font-bold text-[var(--text-primary)]">{item.title}</h3>
                <p className="text-[10px] font-mono text-white/40 uppercase tracking-widest">{item.desc}</p>
              </div>
            </div>

            <div className="mt-6 flex gap-1 h-6 items-end opacity-20 group-hover:opacity-100 transition-opacity">
