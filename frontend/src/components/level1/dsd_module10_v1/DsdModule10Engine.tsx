...
import { Link as LinkIcon, Moon, Sun, ArrowRight, ArrowLeft } from 'lucide-react';
...
const PAGES: Page[] = [
  ...
  { id: 'circuit',  part: 'PART II · THE MECHANISM', label: 'The Full Circuit',      subtitle: 'Gate-level schematic, the carry chain wired live.', Component: S08_Circuit },
...
const getPartTheme = (part: string) => {
  ...
  if (part.startsWith('PART I '))   return { primary: '#f59e0b', secondary: '#fbbf24', glow: 'rgba(245, 158, 11, 0.12)' };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase font-mono tracking-widest font-bold transition-colors duration-500" style={{ color: theme.primary }}>
              Module 10 · The Ripple-Carry Adder
            </p>
          </div>
        </div>
      </header>
...
export const DsdModule10Engine: React.FC<{
  isDarkMode: boolean;
  onThemeToggle: () => void;
  ...
  return (
    <div className={`flex h-screen overflow-hidden transition-colors duration-700 relative ${isDarkMode ? 'bg-[#020100]' : 'bg-white'}`}>
        ...
        >
          ...
          <div className="hidden md:flex items-center gap-8">
            <div className="text-right">
              <div className="text-[8px] font-mono uppercase tracking-widest opacity-30">DSD MODULE 10</div>
      ...
      {done && (
        <ModuleComplete
          isDark={isDarkMode}
          moduleTitle={MODULE_LABELS['dsd/10'] ?? 'this module'}
          accent={theme.primary}
          topics={Array.from(new Set(PAGES.map((p) => p.label)))}
          onPortal={() => navigate('/portal')}
          next={{ label: MODULE_LABELS['dsd/11'] ?? 'Next module', onGo: () => navigate('/dsd/11') }}
        />
      )}
    </div>
  );
