...
import { ChefHat, Moon, Sun, ArrowRight, ArrowLeft } from 'lucide-react';
...
interface Page { id: string; part: string; label: string; subtitle: string; Component: React.FC<any>; }
...
const PAGES: Page[] = [
  ...
  { id: 'circuit',  part: 'PART II · THE MECHANISM', label: 'The Full Circuit',     subtitle: 'Gate-level schematic, the parallel carries wired.', Component: S08_Circuit },
...
const getPartTheme = (part: string) => {
  ...
  if (part.startsWith('PART I '))   return { primary: '#fb923c', secondary: '#fdba74', glow: 'rgba(251, 146, 60, 0.12)' };
  return { primary: '#fb923c', secondary: '#fdba74', glow: 'rgba(251, 146, 60, 0.12)' };
};
    ...
    <div className={`w-[320px] h-full flex-shrink-0 border-r-2 flex flex-col z-20 transition-all duration-700 relative ${isDarkMode ? 'bg-[#040200]' : 'bg-slate-50'}`} style={{ borderColor }}>
      <header className="p-10 border-b" style={{ borderColor }}>
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-2xl bg-orange-400 flex items-center justify-center text-black"><ChefHat size={20} /></div>
          <div>
            <h2 className={`text-sm font-black tracking-tight ${textColor}`}>Digital System Design</h2>
            <p className="text-[10px] uppercase font-mono tracking-widest font-bold transition-colors duration-500" style={{ color: theme.primary }}>Module 11 · Carry Look-Ahead Adder</p>
          </div>
        </div>
      </header>
          ...
          return (
            <React.Fragment key={page.id}>
              {showHeader && (
                <div className="pt-8 pb-3 px-4 first:pt-0">
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 whitespace-nowrap" style={{ color: partTheme.primary }}>{page.part}</span>
              ...
              <button onClick={() => onChange(idx)}
                ...
                <div className={`mt-0.5 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black border transition-all duration-500 ${isDone || isActive ? 'text-black' : isDarkMode ? 'bg-transparent border-white/10 opacity-30' : 'bg-transparent border-slate-300 opacity-40'}`}
      ...
      <footer className="p-10 border-t space-y-6" style={{ borderColor }}>
        ...
        <button onClick={toggleTheme} className={`h-12 w-full rounded-2xl border-2 border-edge shadow-brutal-sm flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest transition-all ${isDarkMode ? 'border-white/10 text-slate-400 hover:bg-white/5' : 'border-slate-200 text-slate-600 hover:bg-slate-100'}`}>
...
export const DsdModule11Engine: React.FC<{ isDarkMode: boolean; onThemeToggle: () => void; initialChapter?: string; }> = ({ isDarkMode, onThemeToggle, initialChapter }) => {
  const navigate = useNavigate();
  const findInitial = useCallback(() => { if (!initialChapter) return 0; const idx = PAGES.findIndex(p => p.id === initialChapter); return idx >= 0 ? idx : 0; }, [initialChapter]);
  ...
  useEffect(() => {
    ...
    if (id) { const target = `/dsd/11/${id}`; if (window.location.pathname !== target) navigate(target, { replace: true }); }
  }, [current, navigate]);

  useEffect(() => {
    if (initialChapter) { const idx = PAGES.findIndex(p => p.id === initialChapter); if (idx >= 0 && idx !== current) setCurrent(idx); }
    else if (current !== 0) setCurrent(0);
  }, [initialChapter]); // eslint-disable-line react-hooks/exhaustive-deps

  const go = useCallback((dir: number) => setCurrent(c => Math.max(0, Math.min(PAGES.length - 1, c + dir))), []);
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative z-10">
        <header className="h-16 lg:h-20 border-b flex items-center justify-between px-4 lg:px-12 z-10 gap-3" style={{ borderColor: 'var(--border-soft)' }}>
          ...
          <div className="hidden md:flex items-center gap-8">
            <div className="text-right">
              <div className="text-[8px] font-mono uppercase tracking-widest opacity-30">DSD MODULE 11</div>
        ...
        <div ref={contentRef} className="flex-1 overflow-y-auto scroll-smooth">
          <AnimatePresence mode="wait">
            <motion.div key={page.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} className="max-w-7xl mx-auto px-4 py-10 lg:px-12 lg:py-24">
        ...
        <footer className="h-20 lg:h-24 border-t flex items-center justify-between px-4 lg:px-12 z-10 gap-3" style={{ borderColor: 'var(--border-soft)' }}>
          <button onClick={() => { if (current === 0) navigate('/portal'); else go(-1); }} className="flex items-center gap-2 px-4 lg:px-8 py-3 rounded-2xl font-bold transition-all hover:bg-black/5 active:scale-95"><ArrowLeft size={18} /> Back</button>
          ...
          <button onClick={() => { if (current === PAGES.length - 1) navigate('/portal'); else go(1); }} className="flex items-center gap-3 px-5 lg:px-10 py-3 rounded-2xl font-black text-black transition-all duration-500 active:scale-95 shadow-xl" style={{ backgroundColor: theme.primary, boxShadow: `0 10px 30px ${theme.primary}33` }}>
