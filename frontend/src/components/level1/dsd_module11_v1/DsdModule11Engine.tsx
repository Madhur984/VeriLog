    ...
    <div className={`flex h-screen overflow-hidden transition-colors duration-700 relative ${isDarkMode ? 'bg-[#020100]' : 'bg-white'}`}>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative z-10">
        ...
        <footer className="h-20 lg:h-24 border-t flex items-center justify-between px-4 lg:px-12 z-10 gap-3" style={{ borderColor: 'var(--border-soft)' }}>
          ...
          <button onClick={() => { if (current === PAGES.length - 1) setDone(true); else go(1); }} className="flex items-center gap-3 px-5 lg:px-10 py-3 rounded-2xl font-black text-black transition-all duration-500 active:scale-95 shadow-xl" style={{ backgroundColor: theme.primary, boxShadow: `0 10px 30px ${theme.primary}33` }}>
      ...
      {done && (
        <ModuleComplete
          isDark={isDarkMode}
          moduleTitle={MODULE_LABELS['dsd/11'] ?? 'this module'}
          accent={theme.primary}
          topics={Array.from(new Set(PAGES.map((p) => p.label)))}
          onPortal={() => navigate('/portal')}
          next={{ label: MODULE_LABELS['dsd/12'] ?? 'Next module', onGo: () => navigate('/dsd/12') }}
        />
      )}
    </div>
  );
