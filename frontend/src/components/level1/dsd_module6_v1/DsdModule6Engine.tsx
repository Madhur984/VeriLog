...
import { Clock, Moon, Sun, ArrowRight, ArrowLeft } from 'lucide-react';
...
import { S05_CombinationalJobs }  from './scenes/S05_CombinationalJobs';
import { S06_TheLimit }           from './scenes/S06_TheLimit';
import { S07_Scoreboard }         from './scenes/S07_Scoreboard';
import { S08_TrapMemory }         from './scenes/S08_TrapMemory';
import { S09_Clock }              from './scenes/S09_Clock';
import { S10_FaceOff }            from './scenes/S10_FaceOff';
...
const PAGES: Page[] = [
  ...
  { id: 'jobs',          part: 'PART II · THE NOW',    label: 'Adders, MUX and Friends',         subtitle: 'Adders, multiplexers and decoders at work.',               Component: S05_CombinationalJobs },
  { id: 'limit',         part: 'PART II · THE NOW',    label: 'The Problem with Now',            subtitle: 'Why memoryless circuits cannot count.',                    Component: S06_TheLimit },
  { id: 'scoreboard',    part: 'PART III · THE THEN',  label: 'The Cricket Scoreboard',          subtitle: 'Past state + present input = next state.',                 Component: S07_Scoreboard },
  { id: 'trap',          part: 'PART III · THE THEN',  label: 'Trapping a Bit',                  subtitle: 'Cross-wired gates lock onto a 1 or 0.',                    Component: S08_TrapMemory },
  { id: 'clock',         part: 'PART III · THE THEN',  label: 'The Heartbeat Clock',             subtitle: 'A metronome that synchronizes every update.',              Component: S09_Clock },
  { id: 'faceoff',       part: 'PART IV · SYNTHESIS',  label: 'Now vs Then - Face Off',          subtitle: 'Side by side · the at-a-glance table.',                    Component: S10_FaceOff },
  { id: 'practice',      part: 'PART IV · SYNTHESIS',  label: 'Practice Arena',                  subtitle: 'Boss drill with instant walkthroughs.',                    Component: S11_Practice },
];

const getPartTheme = (part: string) => {
  // Match longest prefixes first so "IV" doesn't get caught by "I ·" etc.
  if (part.startsWith('PART IV '))  return { primary: '#e879f9', secondary: '#f0abfc', glow: 'rgba(232, 121, 249, 0.12)' };
  if (part.startsWith('PART III ')) return { primary: '#f59e0b', secondary: '#fbbf24', glow: 'rgba(245, 158, 11, 0.12)' };
  if (part.startsWith('PART II '))  return { primary: '#10b981', secondary: '#34d399', glow: 'rgba(16, 185, 129, 0.12)' };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase font-mono tracking-widest font-bold transition-colors duration-500" style={{ color: theme.primary }}>
              Module 06 · Combinational &amp; Sequential
            </p>
          </div>
        </div>
      </header>
...
export const DsdModule6Engine: React.FC<{
  isDarkMode: boolean;
  onThemeToggle: () => void;
  ...
  useEffect(() => {
    if (initialChapter) {
      ...
      // No chapter param → user explicitly wants /dsd/6, start from cover
  ...
  return (
    <div className={`flex h-screen overflow-hidden transition-colors duration-700 relative ${isDarkMode ? 'bg-[#020100]' : 'bg-white'}`}>
        ...
        >
          ...
          <div className="hidden md:flex items-center gap-8">
            <div className="text-right">
              <div className="text-[8px] font-mono uppercase tracking-widest opacity-30">DSD MODULE 06</div>
      ...
      {done && (
        <ModuleComplete
          isDark={isDarkMode}
          moduleTitle={MODULE_LABELS['dsd/6'] ?? 'this module'}
          accent={theme.primary}
          topics={Array.from(new Set(PAGES.map((p) => p.label)))}
          onPortal={() => navigate('/portal')}
          next={{ label: MODULE_LABELS['dsd/7'] ?? 'Next module', onGo: () => navigate('/dsd/7') }}
        />
      )}
    </div>
  );
