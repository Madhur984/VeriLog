...
export function useSigmaMentorL3() {
    ...
    const getProactiveMessage = useCallback((scene: L3Scene): string | null => {
        const { hesitationTime, errorStreak } = metrics;
        if (errorStreak >= 2) return "Engineering Alert: Repeated state mismatch detected. Try analyzing the carry chain at 0.5x speed.";
    ...
    const getResponse = useCallback((scene: L3Scene): SigmaResponse => {
        const tier = getTier();
        if (!SIGMA_DB[scene]) {
            console.warn(`SIGMA_DB missing context: ${scene}`);
            return {
