...
export interface AnalystL4Response {
    observation: string;
    analysis: string;
    conclusion: string;
    insight: string;
...
const DB: Record<L4Scene, Record<L4Tier, AnalystL4Response[]>> = {
    gates: {
        sharp: [
            {
...
export function useLogicAnalystL4() {
    ...
    const getResponse = useCallback((scene: L4Scene): AnalystL4Response => {
        const tier = getTier();
