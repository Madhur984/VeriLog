...
import { ModuleComplete } from '../components/ui/ModuleComplete';
...
const PAGES: Page[] = [
  { id: 'intro',    part: 'PART I · INTRODUCTION', partNum: 3, label: 'Why Logic Gates?',     subtitle: 'The fundamental building blocks.', accentHex: '#0ea5e9', Component: GateIntro },
  { id: 'and',      part: 'PART II · BASIC GATES',  partNum: 3, label: 'AND Gate',             subtitle: 'The logical multiplier.',      accentHex: '#10b981', Component: GateDetail, props: { gateId: 'AND' } },
  { id: 'or',       part: 'PART II · BASIC GATES',  partNum: 3, label: 'OR Gate',              subtitle: 'The logical adder.',           accentHex: '#f59e0b', Component: GateDetail, props: { gateId: 'OR' } },
  { id: 'not',      part: 'PART II · BASIC GATES',  partNum: 3, label: 'NOT Gate',             subtitle: 'The logical inverter.',        accentHex: '#ef4444', Component: GateDetail, props: { gateId: 'NOT' } },
  { id: 'nand',     part: 'PART III · UNIVERSAL',   partNum: 3, label: 'NAND Gate',            subtitle: 'The universal builder.',       accentHex: '#8b5cf6', Component: GateDetail, props: { gateId: 'NAND' } },
  { id: 'nor',      part: 'PART III · UNIVERSAL',   partNum: 3, label: 'NOR Gate',             subtitle: 'The universal refiner.',       accentHex: '#ec4899', Component: GateDetail, props: { gateId: 'NOR' } },
  { id: 'xor',      part: 'PART IV · EXCLUSIVE',    partNum: 3, label: 'XOR/XNOR',             subtitle: 'The difference checkers.',     accentHex: '#06b6d4', Component: GateDetail, props: { gateId: 'XOR' } },
  { id: 'discovery',part: 'PART V · INTERACTIVE',   partNum: 3, label: 'Gate Discovery',       subtitle: 'Universal gate challenge.',    accentHex: '#6366f1', Component: GateDiscovery },
  { id: 'lab',      part: 'PART VI · ENGINEERING',  partNum: 3, label: 'Logic Laboratory',     subtitle: 'Build complex circuits.',      accentHex: '#14b8a6', Component: GateLab },
  { id: 'alu',      part: 'PART VII · ARCHITECTURE',partNum: 3, label: 'The Mini ALU',          subtitle: 'A full 4-function CPU core.',  accentHex: '#f43f5e', Component: ALULab },
];

const getPartTheme = (part: string) => {
  if (part.includes('I ·')) return { primary: '#0ea5e9', secondary: '#3b82f6', glow: 'rgba(14, 165, 233, 0.1)' };
  ...
  if (part.includes('V ·')) return { primary: '#6366f1', secondary: '#4f46e5', glow: 'rgba(99, 102, 241, 0.1)' };
  if (part.includes('VI ·')) return { primary: '#14b8a6', secondary: '#0d9488', glow: 'rgba(20, 184, 166, 0.1)' };
  if (part.includes('VII ·')) return { primary: '#f43f5e', secondary: '#e11d48', glow: 'rgba(244, 63, 94, 0.1)' };
  return { primary: '#0ea5e9', secondary: '#3b82f6', glow: 'rgba(14, 165, 233, 0.1)' };
};
    ...
    <div className={`w-[320px] h-full flex-shrink-0 border-r flex flex-col z-20 transition-all duration-700 relative ${isDarkMode ? 'bg-[#040200]' : 'bg-slate-50'}`} style={{ borderColor }}>
      <header className="p-10 border-b" style={{ borderColor }}>
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-2xl bg-sky-500 flex items-center justify-center text-black">
            <Activity size={20} />
          </div>
          <div>
            <h2 className={`text-sm font-black tracking-tight ${textColor}`}>Digital Logic</h2>
            <p className="text-[10px] uppercase font-mono tracking-widest font-bold transition-colors duration-500" style={{ color: theme.primary }}>Module 03</p>
          </div>
        </div>
      </header>
  ...
  return (
    <div className={`flex h-screen overflow-hidden transition-colors duration-700 relative ${isDarkMode ? 'bg-[#020100]' : 'bg-white'}`}>
      ...
      {done && (
        <ModuleComplete
          isDark={isDarkMode}
          moduleTitle={MODULE_LABELS['module/3'] ?? 'this module'}
          accent={theme.primary}
          topics={Array.from(new Set(PAGES.map((p) => p.label)))}
          onPortal={() => navigate('/portal')}
          next={{ label: MODULE_LABELS['module/4'] ?? 'Module 4', onGo: () => navigate('/module/4') }}
        />
      )}
    </div>
  );
