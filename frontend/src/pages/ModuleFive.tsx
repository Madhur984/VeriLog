...
const PAGES: Page[] = [
  { id: 'intro',       part: 'PART I · INTRODUCTION',         partNum: 5, label: 'Why K-Maps?',        subtitle: 'The Problem of Simplification',       accentHex: '#0EA5E9', kind: 'theory', Component: IntroTheory },
  { id: 'structure-2v',part: 'PART II · MAP CONSTRUCTION',   partNum: 5, label: '2-Variable K-Map',   subtitle: 'Step-by-step grid building',          accentHex: '#10B981', kind: 'theory', Component: TwoVarTheory },
  { id: 'structure-3v',part: 'PART II · MAP CONSTRUCTION',   partNum: 5, label: '3-Variable K-Map',   subtitle: 'Gray Code & Interactive Grid',         accentHex: '#3B82F6', kind: 'theory', Component: ThreeVarTheory },
  { id: 'structure-4v',part: 'PART II · MAP CONSTRUCTION',   partNum: 5, label: '4-Variable K-Map',   subtitle: 'The 4×4 Matrix & Traps',               accentHex: '#8B5CF6', kind: 'theory', Component: FourVarTheory },
  { id: 'rules',       part: 'PART III · RULES OF GROUPING', partNum: 5, label: 'Pairs & All 7 Rules', subtitle: 'Complete guide to grouping minterms',  accentHex: '#F59E0B', kind: 'theory', Component: GroupingRulesTheory },
  { id: 'dont-cares',  part: 'PART III · RULES OF GROUPING', partNum: 5, label: "Don't Care Cells",    subtitle: 'Using X to simplify even more',       accentHex: '#F43F5E', kind: 'theory', Component: DontCareTheory },
  { id: 'pos',         part: 'PART IV · POS SYSTEM',         partNum: 5, label: 'Product of Sums',     subtitle: 'The mirror image (circling 0s)',       accentHex: '#8B5CF6', kind: 'theory', Component: POSTheory },
  { id: 'lab',         part: 'PART V · TRY IT YOURSELF',     partNum: 5, label: 'K-Map Sandbox',       subtitle: 'Interactive K-Map Optimization Lab',   accentHex: '#00D4FF', kind: 'lab',    Component: () => <div className="mt-2 h-[80vh] w-full rounded-2xl overflow-hidden border border-white/10 shadow-[0_0_40px_rgba(0,212,255,0.15)]"><iframe src="http://localhost:3001" className="w-full h-full border-none" title="K-Map Lab" sandbox="allow-scripts allow-same-origin allow-forms" /></div> },
];
  ...
  return (
    <div className={`w-[300px] flex-shrink-0 border-r flex flex-col z-20 overflow-y-auto transition-all duration-500 backdrop-blur-xl ${isDarkMode ? 'bg-gradient-to-b from-[#0B0D14]/90 to-[#050608]/90' : 'bg-gradient-to-b from-slate-50/90 to-white/90'}`} style={{ borderColor }}>
      <header className="p-8 border-b relative overflow-hidden" style={{ borderColor }}>
        <div className={`absolute top-0 left-0 w-full h-full opacity-20 pointer-events-none ${isDarkMode ? 'bg-[radial-gradient(circle_at_20%_20%,#10B981,transparent_60%)]' : ''}`} />
        <div className="flex flex-col gap-4 relative z-10">
            ...
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center border shadow-lg ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : 'bg-emerald-500 shadow-emerald-500/30 border-emerald-600'}`}>
                <Activity className={isDarkMode ? 'text-emerald-400' : 'text-white'} size={18} />
            </div>
            <div>
                <h2 className={`text-sm font-bold tracking-tight ${textColor}`}>Karnaugh Maps</h2>
                <p className={`text-[10px] font-mono uppercase tracking-widest mt-1 font-bold ${subTextColor}`}>Module 05</p>
            </div>
        </div>
      </header>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <div ref={contentRef} className="flex-1 overflow-y-auto overflow-x-hidden relative scroll-smooth">
          <AnimatePresence mode="wait">
            <motion.div key={page.id} initial={{ opacity: 0, scale: 0.98, y: 15 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98, y: -15 }} transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }} className="lg:px-[4vw] xl:px-[8vw] pb-32 pt-8">
