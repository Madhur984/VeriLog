    ...
    <div className={`flex h-screen overflow-hidden ${isDarkMode ? 'bg-matte-obsidian text-oscilloscope-trace' : 'bg-slate-50 text-slate-900'} relative`}>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative z-10">
        <header className="h-16 border-b border-white/10 backdrop-blur-md flex items-center justify-between px-10 z-10 bg-black/20">
          ...
          <div className="hidden md:flex items-center gap-8">
             <div className="text-right border-r pr-8 border-white/5">
                <div className="text-[8px] font-mono uppercase tracking-widest opacity-30 italic">Verilog // Engine</div>
             </div>
             <div className="text-[10px] font-mono opacity-20">{current + 1} / {PAGES.length}</div>
          </div>
        </header>

        <div ref={contentRef} className="flex-1 overflow-y-auto scroll-smooth py-8 px-6 md:px-12 lg:px-20 scrollbar-hide">
          <AnimatePresence mode="wait">
            <motion.div key={page.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.4, ease: "easeOut" }} className="max-w-5xl mx-auto min-h-[calc(100vh-12rem)] flex items-center justify-center">
        ...
        <footer className="h-20 border-t border-white/5 flex items-center justify-between px-10 z-10">
          <button disabled={current === 0} onClick={() => go(-1)} className={`flex items-center gap-3 px-8 py-3 rounded-[20px] font-black uppercase tracking-widest text-[10px] transition-all ${current === 0 ? 'opacity-0' : 'hover:bg-white/5 border border-white/10 text-white/40'}`}>
          ...
          <div className="hidden md:flex flex-col items-center gap-2">
              ...
              <span className="text-[8px] font-mono opacity-20 uppercase tracking-[0.5em]">Neural Link Status: Active</span>
          </div>
          <button onClick={() => go(1)} disabled={current === PAGES.length - 1} className={`flex items-center gap-3 px-10 py-3 rounded-[20px] font-black uppercase tracking-[0.4em] text-[10px] transition-all duration-500 ${current === PAGES.length - 1 ? 'opacity-0' : 'text-black shadow-cyan-glow group'}`}
