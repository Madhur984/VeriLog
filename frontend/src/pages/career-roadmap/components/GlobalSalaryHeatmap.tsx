  ...
  return (
    <DataTerminal title="GLOBAL SILICON HOTSPOTS" subtitle="Real-time PPP Analysis & Entity Presence">
      <div className="relative w-full h-[320px] sm:h-[500px] bg-[#020408] overflow-hidden rounded-sm flex">
        ...
        >
          <svg viewBox="0 0 100 60" className="w-full h-full text-cyan-400 fill-current">
          ...
          <div className="absolute inset-0 bg-[linear-gradient(rgba(0,0,0,0)_1px,transparent_1px),linear-gradient(90deg,rgba(0,0,0,0)_1px,transparent_1px)] bg-[size:40px_40px] opacity-10 border-white" />
        </motion.div>
        ...
        {HOTSPOTS.map((spot) => (
          ...
          >
            <div className="w-2 h-2 bg-cyan-400 rounded-full mx-auto mt-1 group-hover:scale-150 transition-transform" />
            <div className="absolute inset-0 border border-cyan-400 rounded-full animate-ping opacity-50" />
          </motion.div>
        ))}
            ...
            >
              <div className="flex justify-between items-start mb-4">
                 ...
                 <div className="text-right">
                   ...
                   <div className="text-lg font-bold text-white font-mono">{activeSpot.ppp}</div>
                 </div>
              </div>
              
              <div className="space-y-2 pt-4 border-t border-white/10">
                <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">Dominant Entities</div>
                <div className="flex flex-wrap gap-2">
                  {activeSpot.entities.map(e => (
                    <span key={e} className="px-2 py-1 bg-white/5 border border-white/10 text-[10px] font-mono text-slate-300">
                      {e}
                    </span>
                  ))}
                </div>
