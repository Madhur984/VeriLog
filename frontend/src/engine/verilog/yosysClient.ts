...
export type SynthResult = { ok: true; json: string } | { ok: false; error: string };
...
function ensureWorker(): Worker {
  ...
  worker.onmessage = (e: MessageEvent<{ id: number; type: string; json?: string; error?: string; done?: number; total?: number }>) => {
    ...
    if (type === 'progress') {
      ...
      pending.delete(id);
      entry.resolve({ ok: true, json: e.data.json ?? '' });
    } else if (type === 'error') {
      pending.delete(id);
      entry.resolve({ ok: false, error: e.data.error ?? 'Synthesis failed.' });
    }
  };
  worker.onerror = (ev) => {
    const msg = ev.message || 'The Yosys engine failed to load.';
    pending.forEach((entry) => entry.resolve({ ok: false, error: msg }));
    pending.clear();
  };
  return worker;
}
